...
class LicenseBoxAPI{
	...
	public function __construct(){ 
		$this->product_id = 'F94933F9';
		$this->api_url = 'https://licencefly.com/';
		$this->api_key = '3F636961E6D2542-lofuel';
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["73806c3c57bd4ad"])){
				$_SESSION["73806c3c57bd4ad"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["73806c3c57bd4ad"])){
				$get_data = $this->call_api(
					'POST',
